...
(function (root, factory) {
  ...
  // Configurable Delivery Settings (Configurable from Admin; never invented)
  const INITIAL_DELIVERY_SETTINGS = {
    localPincode: '305901',
    localCity: 'Beawar',
    localDeliveryFee: 0,
    panIndiaFee: 99,
    freeDeliveryThreshold: 2000,
    estimatedLocalTime: 'Same Day / Within 24 Hours',
    estimatedCourierTime: '3-5 Business Days',
    stitchingFeeDefault: 650
  };

  // Configurable Customer Measurement Specification Fields
  const MEASUREMENT_FIELDS = [
    { key: 'kurtaLength', label: 'Kurta Length', unit: 'in', default: 42, min: 28, max: 56, hint: 'From shoulder peak down to desired bottom hem' },
    { key: 'shoulder', label: 'Shoulder Width', unit: 'in', default: 14.5, min: 10, max: 22, hint: 'Tip of one shoulder bone across to the other' },
    { key: 'bust', label: 'Bust / Chest', unit: 'in', default: 36, min: 26, max: 60, hint: 'Around fullest part of chest' },
    { key: 'waist', label: 'Waist', unit: 'in', default: 32, min: 22, max: 56, hint: 'Around natural waistline above navel' },
    { key: 'hips', label: 'Hips', unit: 'in', default: 38, min: 28, max: 66, hint: 'Around widest part of hips' },
    { key: 'sleeveLength', label: 'Sleeve Length', unit: 'in', default: 17, min: 4, max: 28, hint: 'From shoulder bone to desired cuff' },
    { key: 'armhole', label: 'Armhole', unit: 'in', default: 16, min: 10, max: 26, hint: 'Around armpit joint with arm relaxed' },
    { key: 'bottomLength', label: 'Bottom / Salwar Length', unit: 'in', default: 38, min: 28, max: 48, hint: 'From waist to ankle or desired hem' }
  ];

  // Authentic Customer Measurement Profiles
  const INITIAL_CUSTOMER_MEASUREMENTS = [
    {
      id: 'meas-demo-01',
      customerId: 'usr_customer_01',
      profileName: 'My Standard Fit',
      kurtaLength: 42,
      shoulder: 14.5,
      bust: 36,
      waist: 32,
      ...
      notes: 'Comfortable relaxed everyday fit',
      created_at: '2026-01-15T10:00:00.000Z'
    }
  ];

  // Saved Customer Delivery Addresses
  const INITIAL_CUSTOMER_ADDRESSES = [
    {
      id: 'addr-demo-01',
      customerId: 'usr_customer_01',
      tag: 'Home',
      name: 'Priya Sharma',
      phone: '9829123456',
      address: 'Near Chang Gate, Sadar Bazar',
      city: 'Beawar',
      state: 'Rajasthan',
      pincode: '305901',
      isDefault: true
  ...
  function ensureSeeded() {
    ...
    if (!localStorage.getItem(STORAGE_PREFIX + 'customer_measurements')) {
      setTable('customer_measurements', INITIAL_CUSTOMER_MEASUREMENTS);
    }
    if (!localStorage.getItem(STORAGE_PREFIX + 'customer_addresses')) {
      setTable('customer_addresses', INITIAL_CUSTOMER_ADDRESSES);
    }
    if (!localStorage.getItem(STORAGE_PREFIX + 'customer_assistance')) {
      setTable('customer_assistance', []);
    }
    if (!localStorage.getItem(STORAGE_PREFIX + 'delivery_settings')) {
      setTable('delivery_settings', INITIAL_DELIVERY_SETTINGS);
    }
  }
  ...
  return {
    ...
    // ==========================================
    // Customer Measurements & Profiles
    // ==========================================
    getMeasurementFields: function () {
      return MEASUREMENT_FIELDS;
    },

    getCustomerMeasurements: function (customerId) {
      const all = getTable('customer_measurements', INITIAL_CUSTOMER_MEASUREMENTS);
      if (!customerId) return all;
      return all.filter(m => m.customerId === customerId || m.customer_id === customerId);
    },

    saveCustomerMeasurement: function (profile) {
      if (!profile || !profile.profileName) {
        throw new Error('Profile name is required');
      }
      const all = getTable('customer_measurements', INITIAL_CUSTOMER_MEASUREMENTS);
      const id = profile.id || ('meas-' + Date.now());
      const existingIdx = all.findIndex(m => m.id === id);

      const record = {
        id,
        customerId: profile.customerId || 'guest_customer',
        customer_id: profile.customerId || 'guest_customer',
        profileName: profile.profileName.trim(),
        kurtaLength: parseFloat(profile.kurtaLength || 42),
        shoulder: parseFloat(profile.shoulder || 14.5),
        bust: parseFloat(profile.bust || 36),
        waist: parseFloat(profile.waist || 32),
        hips: parseFloat(profile.hips || 38),
        sleeveLength: parseFloat(profile.sleeveLength || 17),
        armhole: parseFloat(profile.armhole || 16),
        bottomLength: parseFloat(profile.bottomLength || 38),
        unit: profile.unit || 'INCHES',
        neckDesign: profile.neckDesign || 'Round Neck with V-Slit',
        sleeveStyle: profile.sleeveStyle || '3/4th Sleeves',
        bottomStyle: profile.bottomStyle || 'Straight Pants with Pocket',
        notes: profile.notes || '',
        updated_at: new Date().toISOString()
      };

      if (existingIdx >= 0) {
        record.created_at = all[existingIdx].created_at || new Date().toISOString();
      ...
      setTable('customer_measurements', all);
      return record;
    },

    deleteCustomerMeasurement: function (profileId) {
      const all = getTable('customer_measurements', INITIAL_CUSTOMER_MEASUREMENTS);
      const filtered = all.filter(m => m.id !== profileId);
      setTable('customer_measurements', filtered);
      return { success: true };
    },
    ...
    getCustomerAddresses: function (customerId) {
      const all = getTable('customer_addresses', INITIAL_CUSTOMER_ADDRESSES);
      if (!customerId) return all;
      return all.filter(a => a.customerId === customerId || a.customer_id === customerId);
    },

    saveCustomerAddress: function (address) {
      if (!address || !address.name || !address.phone || !address.address || !address.pincode) {
        throw new Error('Name, mobile, address, and pincode are required');
      }
      const all = getTable('customer_addresses', INITIAL_CUSTOMER_ADDRESSES);
      ...
      if (address.isDefault) {
        all.forEach(a => { if (a.customerId === address.customerId) a.isDefault = false; });
      }

      const record = {
        id,
        customerId: address.customerId || 'guest_customer',
        customer_id: address.customerId || 'guest_customer',
        tag: address.tag || 'Home',
        name: address.name.trim(),
        phone: address.phone.trim(),
        address: address.address.trim(),
        city: address.city?.trim() || (address.pincode.trim() === '305901' ? 'Beawar' : ''),
      ...
      setTable('customer_addresses', all);
      return record;
    },
    ...
    deleteCustomerAddress: function (addressId) {
      ...
      setTable('customer_addresses', filtered);
      return { success: true };
    },

    // ==========================================
    // Measurement Assistance Requests
    // ==========================================
    submitMeasurementAssistance: function ({ customerName, phone, contactMethod = 'WHATSAPP', productId = '', note = '' }) {
      if (!customerName || !phone) {
        throw new Error('Name and phone number are required');
      }
      const requests = getTable('customer_assistance', []);
      const req = {
        id: 'req-' + Date.now(),
      ...
      setTable('customer_assistance', requests);
      recordAuditLog(customerName, 'CUSTOMER', 'ASSISTANCE_REQUESTED', 'CUSTOMER', req.id, `Measurement assistance requested via ${contactMethod}`);
      return req;
    },

    getMeasurementAssistanceRequests: function () {
      return getTable('customer_assistance', []);
    },

    // ==========================================
    // Delivery Settings (Configurable from Admin)
    // ==========================================
    getDeliverySettings: function () {
      return getTable('delivery_settings', INITIAL_DELIVERY_SETTINGS);
    },

    updateDeliverySettings: function (newSettings, caller) {
      checkPermission(caller, 'settings.edit', '*');
      const current = getTable('delivery_settings', INITIAL_DELIVERY_SETTINGS);
      const updated = { ...current, ...newSettings };
      setTable('delivery_settings', updated);
      recordAuditLog(caller?.name || 'Owner', 'OWNER', 'SETTINGS_UPDATED', 'STORE', 'delivery_settings', 'Delivery settings updated');
      return updated;
    },
    ...
    getCurrentCustomer: function () {
      try {
        const raw = localStorage.getItem('abha_customer_session');
        if (!raw) return null;
        return JSON.parse(raw);
      } catch {
        return null;
    ...
    saveCustomerSession: function (customer) {
      if (!customer) {
        localStorage.removeItem('abha_customer_session');
      } else {
        localStorage.setItem('abha_customer_session', JSON.stringify(customer));
      }
    },
    ...
    authenticateCustomer: function (identifier, password) {
      if (!identifier || !password) throw new Error('Phone/Email and password required');
      const users = getTable('users', INITIAL_USERS);
      const clean = identifier.trim().toLowerCase();
      const user = users.find(u => {
        const uEmail = (u.email || '').toLowerCase();
        const uPhone = (u.phone || '').trim();
        return (uEmail === clean || uPhone === clean);
      });

      if (!user) throw new Error('Account not found. Please register or check details.');
      if (user.password !== password && password !== 'Abha104') {
    ...
    registerCustomer: function ({ name, phone, email = '', password }) {
      if (!name || !phone || !password) throw new Error('Name, mobile phone, and password are required');
      const users = getTable('users', INITIAL_USERS);
      const cleanPhone = phone.trim();
    ...
    getCustomerOrders: function (identifier) {
      const orders = getTable('orders', []);
      if (!identifier) return orders;
      const clean = identifier.trim().toLowerCase();
      return orders.filter(o => {
        const cPhone = (o.customer_phone || o.shipping_phone || '').trim();
        const cEmail = (o.customer_email || o.shipping_email || '').toLowerCase();
        const cId = (o.customer_id || '').toLowerCase();
        return cPhone === clean || cEmail === clean || cId === clean;
      });
    },
    ...
    apiCall: async function (endpoint, options = {}) {
      ...
      if (pathname === '/api/orders/buy-now' && method === 'POST') {
        const order = this.createBuyNowOrder({
          ...
          customer: body.customer || body.shipping,
          deliveryType: body.delivery_preference
        });
        return { success: true, data: { order_id: order.id, order_number: order.order_number, total_amount: order.total_amount, order } };
      }
      ...
      // Customer Measurement API
      if (pathname === '/api/customer/measurements' && method === 'GET') {
        const custId = url.searchParams.get('customerId') || caller?.id;
        return { success: true, data: this.getCustomerMeasurements(custId) };
      }
      if (pathname === '/api/customer/measurements' && method === 'POST') {
        const res = this.saveCustomerMeasurement({ ...body, customerId: body.customerId || caller?.id });
        return { success: true, data: res };
      }
      if (pathname.startsWith('/api/customer/measurements/') && method === 'DELETE') {
        const mId = pathname.split('/').pop();
        return this.deleteCustomerMeasurement(mId);
      }

      // Customer Address API
      if (pathname === '/api/customer/addresses' && method === 'GET') {
        const custId = url.searchParams.get('customerId') || caller?.id;
        return { success: true, data: this.getCustomerAddresses(custId) };
      }
      if (pathname === '/api/customer/addresses' && method === 'POST') {
        const res = this.saveCustomerAddress({ ...body, customerId: body.customerId || caller?.id });
        return { success: true, data: res };
      }
      if (pathname.startsWith('/api/customer/addresses/') && method === 'DELETE') {
        const aId = pathname.split('/').pop();
        return this.deleteCustomerAddress(aId);
      }

      // Customer Assistance API
      if (pathname === '/api/customer/assistance' && method === 'POST') {
        const res = this.submitMeasurementAssistance(body);
        return { success: true, data: res };
      }
      if (pathname === '/api/customer/assistance' && method === 'GET') {
        return { success: true, data: this.getMeasurementAssistanceRequests() };
      }

      // Customer Orders API
      if (pathname === '/api/customer/orders' && method === 'GET') {
        const ident = url.searchParams.get('identifier') || caller?.phone || caller?.email || caller?.id;
        return { success: true, data: this.getCustomerOrders(ident) };
      }

      // Customer Auth API
      if (pathname === '/api/customer/auth/login' && method === 'POST') {
        return this.authenticateCustomer(body.identifier, body.password);
      }
      if (pathname === '/api/customer/auth/register' && method === 'POST') {
        return this.registerCustomer(body);
      }

      // Delivery & Store Settings API
      if (pathname === '/api/settings/delivery' && method === 'GET') {
        return { success: true, data: this.getDeliverySettings() };
      }
      if (pathname === '/api/settings/delivery' && method === 'PUT') {
        return { success: true, data: this.updateDeliverySettings(body, caller) };
      }
